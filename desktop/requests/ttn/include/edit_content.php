...
$requestConnectedT=$DB->query("SELECT * FROM `request_transport` WHERE `type`=1 AND `status`=3 AND `company_doer`='".$_SESSION["company_id"]."'");
while ($requestConnected=$requestConnectedT->fetch_assoc()) {
    array_push($pageData["request_connected"], array("value"=>$requestConnected["id"],"text"=>"ID".$requestConnected["id"]." від ".date("d.m.Y", $requestConnected["created"])));
}
...
$pageContent[0]=createFormRow(array(
    ...
    createFormBlock("col-xl-3 col-md-4 col-12", array(
        createFormItem("input", array(
            ...
            "value" => date("d.m.Y", $request["doc_date"]),
